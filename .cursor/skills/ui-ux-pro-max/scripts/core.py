...
# Drop domains/stacks whose CSV is not present under data/ (CHT may trim the dataset).
CSV_CONFIG = {
    key: cfg
    for key, cfg in CSV_CONFIG.items()
    if (DATA_DIR / cfg["file"]).is_file()
}
STACK_CONFIG = {
    key: cfg
    for key, cfg in STACK_CONFIG.items()
    if (DATA_DIR / cfg["file"]).is_file()
}
...
def _load_product_keywords():
    ...
    product_cfg = CSV_CONFIG.get("product")
    if not product_cfg:
        return seed

    filepath = DATA_DIR / product_cfg["file"]
    if not filepath.exists():
        return seed
    try:
...
def _domain_keywords():
    ...
    product_cfg = CSV_CONFIG.get("product")
    product_path = DATA_DIR / product_cfg["file"] if product_cfg else None
    signature = _file_signature(product_path) if product_path and product_path.exists() else None
    ...
    # Only route to domains that still have a data file.
    _DOMAIN_KEYWORDS = {
        domain: keywords
...
def detect_domain(query, return_scores=False):
    ...
    fallback = "ux" if "ux" in CSV_CONFIG else (next(iter(CSV_CONFIG)) if CSV_CONFIG else "ux")
    if not ranked:
        return (fallback, None) if return_scores else fallback

    best_domain, best_score = ranked[0]
    result = best_domain if best_score > 0 else fallback
...
def search(query, domain=None, max_results=MAX_RESULTS, diagnostics=False):
    ...
    if not CSV_CONFIG:
        return {"error": "No search domains available (data/*.csv missing)", "domain": domain}
    if domain is not None and domain not in CSV_CONFIG:
        return {
            "error": (
                f"Domain '{domain}' unavailable (CSV missing under data/). "
                f"Available: {', '.join(sorted(CSV_CONFIG))}"
            ),
            "domain": domain,
        }
    ...
    fallback = "ux" if "ux" in CSV_CONFIG else next(iter(CSV_CONFIG))
    search_domain = domain if domain in CSV_CONFIG else fallback
