...
# Only enforce provenance for datasets that are actually present (CHT may trim data/).
_CORE_PROVENANCE_CANDIDATES = {
...
CORE_PROVENANCE_FILES = {
    name for name in _CORE_PROVENANCE_CANDIDATES if (DATA_DIR / name).is_file()
}
CATALOG_PROVENANCE_FILES = {
    name for name in ("google-fonts.csv", "phosphor-icons-upstream.json")
    if (DATA_DIR / name).is_file()
}
_CATALOG_PROVENANCE_ID_FILES = {
    "google-fonts-catalog-2026-08-13": "google-fonts.csv",
    "phosphor-icons-catalog-2.1.1": "phosphor-icons-upstream.json",
}
CATALOG_PROVENANCE_IDS = {
    entity_id
    for entity_id, filename in _CATALOG_PROVENANCE_ID_FILES.items()
...
def _check_reasoning_contract(products, colors, reasoning, style_ids, patterns, problems):
    semantic_sets = [("products", products, "Product Type"), ("reasoning", reasoning, "UI_Category")]
    if "color" in CSV_CONFIG:
        semantic_sets.insert(1, ("colors", colors, "Product Type"))
...
def _row_count(relative):
    """Return CSV row count, or None when the file is absent."""
    path = DATA_DIR / relative
    if not path.is_file():
        return None
    return len(_read_rows(path)[1])


def _load_catalog_json(name, problems, required=True):
    path = DATA_DIR / name
    if not path.is_file():
        if required:
            problems.append(f"[catalog:{name}] missing file")
        return {}
    try:
...
def _check_catalog_summary(summary, licenses, phosphor, problems):
    ...
    styles = _read_rows(styles_path)[1] if styles_path.is_file() else []
    expected = {
        "styles": {
            "total": len(styles),
        ...
        "stackGuidelines": sum(
            len(_read_rows(DATA_DIR / config["file"])[1]) for config in STACK_CONFIG.values()
        ),
    }
    optional_counts = {
        "products": _row_count("products.csv"),
        "palettes": _row_count("colors.csv"),
        "reasoningProfiles": _row_count(REASONING_FILE),
        "fontPairings": _row_count("typography.csv"),
        "googleFonts": _row_count("google-fonts.csv"),
        "curatedIcons": _row_count("icons.csv"),
        "uxGuidelines": _row_count("ux-guidelines.csv"),
        "motionPresets": _row_count("motion.csv"),
        "chartTypes": _row_count("charts.csv"),
    }
    for key, value in optional_counts.items():
        if value is not None:
            expected[key] = value
    if phosphor:
        expected["upstreamPhosphorIcons"] = phosphor.get("iconCount")
...
def _check_catalog_contract(domain_rows, problems):
    ...
    # Catalog artifacts are optional when the CHT dataset is trimmed.
    licenses = _load_catalog_json("google-font-licenses.json", problems, required=False)
    phosphor = _load_catalog_json("phosphor-icons-upstream.json", problems, required=False)
    summary = _load_catalog_json("catalog-summary.json", problems, required=False)
    if licenses:
...
def _check_core_data_contract(domain_rows, problems):
    if "color" in domain_rows:
        _check_color_contract(domain_rows["color"], problems)
    if "chart" in domain_rows:
        _check_chart_contract(domain_rows["chart"], problems)
    if "typography" in domain_rows:
        _check_typography_contract(domain_rows["typography"], problems)
    if "icons" in domain_rows:
        _check_icon_contract(domain_rows["icons"], problems)
    if "ux" in domain_rows:
        _check_ux_contract(domain_rows["ux"], problems)
    if "gsap" in domain_rows:
        _check_motion_contract(domain_rows["gsap"], problems)
    if "web" in domain_rows:
        _check_app_interface_contract(domain_rows["web"], problems)
    if "react" in domain_rows:
        _check_react_contract(domain_rows["react"], problems)
    if "landing" in domain_rows:
        _check_landing_claims(domain_rows["landing"], problems)
...
def _check_provenance(reasoning, styles, problems):
    ...
    if not path.is_file():
        # Optional when the CHT dataset omits provenance metadata.
        return
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
