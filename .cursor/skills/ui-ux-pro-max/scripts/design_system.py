...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        # Step 1: First search product to get category (skip if products.csv trimmed)
        if "product" in CSV_CONFIG:
            product_result = search(query, "product", 1)
        else:
            product_result = {"results": [], "domain": "product", "query": query}
        ...
        motion_snippet = {}
        if motion_info and "gsap" in CSV_CONFIG:
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    # Search across available domains for page-specific guidance
    style_search = (
        search(combined_context, "style", max_results=1)
        if "style" in CSV_CONFIG else {"results": []}
    )
    ux_search = (
        search(combined_context, "ux", max_results=3)
        if "ux" in CSV_CONFIG else {"results": []}
    )
    landing_search = (
        search(combined_context, "landing", max_results=1)
        if "landing" in CSV_CONFIG else {"results": []}
    )
